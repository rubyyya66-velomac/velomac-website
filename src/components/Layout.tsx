...
export function SectionHeader({
  ...
  return (
    <div className={`max-w-4xl ${className}`}>
      ...
      <h2 className="mt-3 text-2xl font-semibold tracking-normal text-navy-950 sm:text-4xl lg:text-[2.75rem] lg:leading-[1.08]">
        {title}
      </h2>
      {description ? (
        <p className="mt-3 text-base leading-7 text-slate-600 sm:mt-5 sm:text-lg">{description}</p>
      ) : null}
    </div>
  );
}
  ...
  return (
    <div className="border-b border-navy-800 bg-navy-950 text-white blueprint-surface dark-technical-grid">
      <Container className="py-[var(--editable-section-spacing-mobile)] sm:py-[var(--editable-section-spacing-tablet)] lg:py-[var(--editable-section-spacing-desktop)]">
        <div className="max-w-4xl border-l-2 border-industrial-400 pl-5 sm:pl-8">
          {eyebrow ? (
          ...
          <p className="mt-3 max-w-4xl text-lg leading-8 text-slate-300 sm:mt-5 sm:text-xl">{description}</p>
        </div>
      </Container>
    </div>
  );
